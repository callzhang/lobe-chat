...
-- ParadeDB BM25 indexes are optional. Managed Postgres providers can reject
-- pg_search, so skip these performance indexes when the bm25 access method
-- is unavailable and allow later schema migrations to continue.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_am WHERE amname = 'bm25') THEN
    RAISE NOTICE 'Skipping optional BM25 indexes because pg_search/bm25 is unavailable';
    RETURN;
  END IF;
